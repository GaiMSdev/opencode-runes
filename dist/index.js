...
// rune_commit helpers
// ---------------------------------------------------------------------------
function determineType(_diff, added, deleted, modified, allFiles) {
    ...
    if (paths.includes("ci/") || paths.includes(".github/") || paths.includes("Dockerfile") || paths.includes("docker-compose"))
        return "ci";
    ...
    if (configFiles.length === all.length && all.length > 0)
        return "chore";
    // Package/dep changes
    if (paths.includes("package.json") || paths.includes("Cargo.toml") || paths.includes("Podfile") || paths.includes("go.mod"))
        return "build";
    ...
    if (fixScore > featScore && fixScore > refactorScore)
        return "fix";
    if (featScore > fixScore && featScore > refactorScore)
        return "feat";
    if (refactorScore > fixScore && refactorScore > featScore)
        return "refactor";
    // By default: feat for new files, fix for small modifications, else chore
    if (added.length > modified.length && added.length > 0)
        return "feat";
...
function determineScope(files) {
    ...
    if (dirs.size === 1)
        return [...dirs][0];
    if (dirs.size <= 2 && dirs.size > 0)
        return [...dirs].join(",");
    return null;
}
function generateSubject(type, _scope, added, deleted, modified, allFiles, _addedLines, _removedLines) {
    ...
    if (type === "test") {
        if (added.length > 0)
            return `add tests for ${added[0].split("/")[0]}`;
        return "update tests";
    }
    if (type === "docs") {
        const docFile = allFiles.find(f => f.endsWith(".md"));
        if (docFile)
            return `update ${docFile.split("/").pop()}`;
    ...
    if (type === "chore") {
        if (deleted.length > 0)
            return `remove ${deleted.length > 1 ? "files" : deleted[0].split("/").pop()}`;
...
- Conventional Commits types: feat, fix, chore, refactor, test, docs, style, perf, ci, build`,
                ...
                async execute() {
                    ...
                    for (const line of diff.split("\n")) {
                        ...
                        if (renameFrom)
                            rewrites.push(`renamed: ${renameFrom[1]}`);
                    ...
                    if (lineCount > 20 || modified.length > 3 || added.length > 1 || deleted.length > 1) {
                        const bulletParts = [];
                        if (added.length > 0)
                            bulletParts.push(`add: ${added.join(", ")}`);
                        if (modified.length > 0)
                            bulletParts.push(`mod: ${modified.join(", ")}`);
                        if (deleted.length > 0)
                            bulletParts.push(`del: ${deleted.join(", ")}`);
                        if (renamed.length > 0)
                            bulletParts.push(renamed.join(", "));
...
No praise, no style/formatting nits.`,
                ...
                async execute({ ref }) {
                    ...
                    const severityOrder = { CRITICAL: 0, WARN: 1, NOTE: 2 };
